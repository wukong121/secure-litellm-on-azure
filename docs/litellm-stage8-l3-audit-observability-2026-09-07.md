...
> 2026-09-10适用范围：本文保留2026-09-07自建L3设计与验证事实，仅适用于可选增强分支。当前第一阶段选择原生Spend Logs，正文进入私有PG，不默认部署本篇Blob/HSM/恢复/双审批平台。原生配置、查询及阶段门禁待适配，不能用本文合成L3测试代替；见[当前部署指南](customer-deployment-workflows-zh.md)。已有正文、Key、保全和访问约束不自动取消。
