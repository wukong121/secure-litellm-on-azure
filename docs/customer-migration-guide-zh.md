...
**入口Service一直Pending、`lb-ready-api`超时的升级恢复：** 仅在AKS cloud-controller日志或Service事件确认`AuthorizationFailed`指向`stage4Network.ingressSubnetName`时使用本流程。需要授权的是实际AKS资源的系统分配控制面身份`identity.principalId`，不是kubelet身份、workload identity、Runner VM或`AZURE_RUNTIME_CLIENT_ID`。新建环境的platform模板已自动在精确入口子网上授予该身份内置`Network Contributor`；已经部署过platform的环境使用独立`aks-ingress-role`组件修复，不重放完整platform。组件从现有platform配置派生AKS、VNet和入口子网名称，并从实际AKS读取控制面Principal ID，`CUSTOMER_CONFIG_JSON`不新增字段。

代码合并到受保护main并按第3节处理新SHA证据后，新建`Customer infrastructure deployment`：environment=test、stage=4、component=aks-ingress-role、operation=plan，release不勾选，两个approved字段和confirm_environment留空。解密并审核`infrastructure-test-4-aks-ingress-role-<run ID>`；本故障的首次修复计划应只有入口子网下的一条`Microsoft.Authorization/roleAssignments` Create，scope形如`.../virtualNetworks/<vnet>/subnets/<ingress-subnet>/providers/Microsoft.Authorization/roleAssignments/<guid>`，角色为内置`Network Contributor`，principalId等于实际AKS `identity.principalId`。出现AKS、VNet、Subnet修改，其他scope/角色、Delete或Unsupported时停止；不能为通过What-if扩大到VNet、目标RG或订阅级Network Contributor。
