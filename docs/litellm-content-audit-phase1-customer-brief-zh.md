...
> 2026-09-10方案决策：项目第一阶段已选择本文的原生Spend Logs方向，自建L3列为可选增强；客户采集范围、读取者、期限和故障策略仍需批准。部署/迁移及[成本清单](litellm-bom-cost-comparison-zh.md)已对齐口径，原生配置/发布门禁与查询接线尚待代码适配，本说明不代表日志已经开启。
...
flowchart TB
    ...
    edge --> auth["企业身份准入\n企业 Token + 客户端 vkey / 请求边界"]
    auth --> gateway["私网 LiteLLM 网关\n按 vkey / 用户 / Team 授权模型与预算\n模型路由 / 原生调用日志"]
...
API采用双凭据：企业Token证明调用者获准进入网关，客户端vkey由LiteLLM验证并决定模型、Team、预算权限。认证代理不再维护模型权限列表，也不代用户选择内部Key。外部人员仅拿到vkey不足以调用，但前提是没有绕过认证层的入口；同时转借企业Token或由内部应用代转请求仍有风险。当前不强制企业主体与Key归属一致，记录和调查时应分别识别实际主体与Key所有者。
...
| --- | --- | --- |
| 身份与责任归属 | 企业准入Token + 客户端vkey；实际主体与Key指纹关联，Key权限由LiteLLM管理 | 降低单独外泄vkey被外部直接使用的风险；不把Key所有者等同实际调用者 |
...
| 模型与用量控制 | LiteLLM统一执行模型授权、预算、速率和并发限制；认证代理不复制模型ACL | 结合日志调查异常用量，不把费用统计当作内容判定 |
...
| 用户、应用、Key、Team 关联 | 确认责任主体与部门 | 实际企业主体来自已验证Token，Key/Team归属来自LiteLLM；二者不保证一致，不信任客户端自填的 user/metadata |
...
| API双凭据与归因 | 缺企业Token或vkey均拒绝；无效Token不进入后台，越权模型/撤销Key/预算限制由LiteLLM拒绝；实际主体与Key归属分别可关联，验证Token自动续期及无旁路 |
...
- API双凭据请求处理及生成器已切换：Authorization携带企业Token，X-LiteLLM-API-Key携带客户端vkey；API模型ACL与内部Key挂载已移除，管理认证保持不变。旧API配置需迁移，真实客户端续期及Entra/模型权限尚待验收，详见[当前认证契约](../auth-proxy/README_ZH.md)。此次不自动清理旧Key/Vault/RBAC，也不等于原生内容日志模式已完成。
