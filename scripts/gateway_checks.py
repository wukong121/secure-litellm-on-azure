...
def gateway_checks(config, revision, resolve=resolve_addresses, request=probe):
    ...
    for name, check in (
        ...
        ("admin_unapproved_source_denied", (admin, "/ui/", admin, {403}, "public")),
    ):
        ...
        try:
            ...
            require(address_policy != "public" or all(not private_address(address) for address in addresses), "Admin DNS must resolve to the public IP-gated edge, not a private origin")
    ...
    return {"revision": revision, "environment": config["environment"], "configSha256": stage_fingerprint(config, 9), "observedAt": datetime.now(timezone.utc).isoformat(), "checkGroup": "gateway-isolation", "checks": results, "stageAccepted": False,
            "notCovered": ["authenticated_client_compatibility", "approved_admin_source_password_login", "object_ownership", "database_and_redis", "audit_delivery", "load_and_recovery", "conditional_access"]}
