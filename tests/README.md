...
The Entra proxy path accepts a restricted set of enterprise-authorized protocols; the native path accepts only exact inference POST routes, virtual keys, and private username/password administration. Legacy tests for Azure-style paths, image/video, WebSocket or encrypted multi-turn references must not be treated as proof of either secure gateway path, nor used to bypass policy. Phase one selects native Spend Logs: validate retained content, actor/Key correlation, access control, cleanup, capacity, backups and write failures. Custom L3 recovery/governance tests apply only to the optional Entra branch. Native runtime and stage gates are implemented, but real virtual-key denial tests, private admin isolation, Front Door header binding, and UI-password delivery still require customer-environment acceptance. Required protocols, private DNS/identity, database migration and rollback remain mandatory; do not skip the current [customer stage gates](../docs/customer-migration-guide-zh.md).
