...
The new proxy accepts a restricted set of Entra-authorized routes. Legacy tests for Azure-style paths, image/video, WebSocket or encrypted multi-turn references must not be treated as proof of secure gateway support, nor used as a reason to bypass policy. Real tenant authorization, required Codex protocols, private DNS/identity, database migration, durable L3 recovery and rollback must pass the relevant [customer stage gates](../docs/customer-migration-guide-zh.md).
