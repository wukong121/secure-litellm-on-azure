...
Entra代理路径只允许有限的企业授权协议；native路径只允许固定推理POST路径、virtual key，以及通过独立Front Door严格mTLS后的用户名/密码Admin。旧网关的Azure风格路径、图像/视频、WebSocket或加密多轮测试结果不能证明任一新入口兼容，更不能作为绕过策略的理由。第一阶段已选择原生Spend Logs，须验实际正文/身份与Key关联、查询权限、清理/容量/备份和写入故障；自建L3恢复/治理测试仅适用于Entra可选增强，不能代替原生落库验收。native配置和阶段门禁已适配，但真实virtual-key拒绝矩阵、Admin客户端证书有效/缺失/错误FQDN/过期/吊销矩阵、双平面Front Door Header绑定和UI密码交付仍须按指南验收。真实客户协议、私网回源/身份、数据库迁移和回退仍须通过对应[阶段门禁](../docs/customer-migration-guide-zh.md)。
